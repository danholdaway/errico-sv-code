c
c  Changes made to MSAR1, MSTAR3, MSTAR4
...
c  Fill and unscale ps 
      nx=nxp
coldqqq      fac=afac*Pnorm/sqrt(R*Tnorm)
      fac=afac/sqrt(pvars)  !qqqqqqqqq
      do 1 j=2,nj2
      do 1 i=2,ni2
      nx=nx+1
...
c  Fill and unscale pt
      nx=nxt
coldqqq      fac1=afac*sqrt(Tnorm/Cp)
      do 2 k=1,nk
coldqqq      fac=fac1/sqrt(dsig(k)) 
      fac=afac/sqrt(tvars(k))  !qqqqqqqqqqqqqq 
      do 2 j=2,nj2
      do 2 i=2,ni2
      nx=nx+1
c      if ( iset3.eq.1 ) then 
...
c  Fill and unscale pu and pv
      do 3 k=1,nk
coldqqq      fac=afac/sqrt(dsig(k)) 
      fac=afac/sqrt(uvars(k))  !qqqqqqqqqqqqqq 
      do 3 j=2,nj1
      do 3 i=2,ni1
      nxu=nxu+1
      nxv=nxv+1
...
c  Fill and unscale ps 
      nx=nxp
coldqqq      fac=afac*Pnorm/sqrt(R*Tnorm)
      fac=afac/sqrt(pvars)  !qqqqqqqqqqqqqq  
      do 1 j=2,nj2
      do 1 i=2,ni2
      nx=nx+1
